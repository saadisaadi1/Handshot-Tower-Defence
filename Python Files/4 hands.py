    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            for handLms in results.multi_hand_landmarks:
                ...
                mfk_distance = depth_image_flipped[y, x] * depth_scale
                distance = mfk_distance
                if distance > 0:
                    if (hand_side == "Right"):
                        if distance < min_right_depth:
                            min_right_depth = distance
                            handlms_right = handLms
                        else:
                            continue
                    if (hand_side == "Left"):
                        if distance < min_left_depth:
                            min_left_depth = distance
                            handlms_left = handLms
                        else:
                            continue
                ...
                if hand_side == "Left" and y > 0:
                    ...
                    y_right_angel = y
            if handlms_left != None:
                mpDraw.draw_landmarks(images, handlms_left, mpHands.HAND_CONNECTIONS)
            if handlms_right != None:
                mpDraw.draw_landmarks(images, handlms_right, mpHands.HAND_CONNECTIONS)
